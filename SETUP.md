...
## 5. Трендтегі ән (Deezer API — автоматты, шексіз, кілтсіз)

Бұл арнаның ерекшелігі — әр видео **шын трендтегі әннің 30 секундтық ресми preview-ін** қолданады, жалпы royalty-free фон музыка емес.

- `video_gen.py` әр жүктеу алдында [Deezer Chart API](https://api.deezer.com/chart/0/tracks) арқылы қазіргі global trending тізімінен кездейсоқ трек таңдайды, оның 30 сек ресми preview mp3-ін жүктейді.
- Сценарий (Gemini) сол нақты ән атауы мен әртісіне негізделіп жазылады — "трендтегі ән туралы" контент.
- Видео сипаттамасына (description) ән атауы, әртісі және Deezer сілтемесі автоматты қосылады (ашықтық үшін).
- **Кілт/тіркелу керек емес** — API толық анонимды жұмыс істейді.

**⚠️ Копирайт тәуекелі (маңызды):** 30 секундтық ресми preview қолдансақ та, бұл әлі де copyrighted material — YouTube Content ID оны танып, видеоны монетизациядан алып тастауы немесе тіпті claim қоюы мүмкін (әдетте strike емес, тек монетизация артист/лейблге бөлінеді). Бұл сіз таңдаған тәуекел деп саналады. Егер бұл жиі орын алса, балама ретінде тек royalty-free "music news" стиліндегі фон әуенге көшуге болады (`music/fallback*.mp3` — CC0/CC-BY, толық қауіпсіз).

**Сақтық fallback:** Deezer/желі сәтсіз болған сирек жағдайға арнап, `music/` папкасында 3 CC-лицензиялы сақтық трек бар (AITechShorts-тан көшірілген, `fallback_attribution.json`-мен бірге).
