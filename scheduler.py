...
# Күнде 3 уақыт (UTC). Психология және AI/Tech боттарымен соқтығыспас үшін ығыстырылған.
# Қазақстан UTC+5:
POST_TIMES = [
    t.strip()
    for t in os.getenv('POST_TIMES', '04:00,10:00,16:00').split(',')
    if t.strip()
]
...
def make_job(slot_num):
    def job():
        ...
        send_telegram(f"⏰ <b>Music {slot_num}/3 жүктеу басталды</b>")
        try:
            generate_video()
...
logger.info(f"🚀 Music Scheduler іске қосылды — күнде {len(POST_TIMES)} видео")
logger.info("   Тоқтату үшін Ctrl+C")
send_telegram(
    f"🚀 <b>Music Scheduler іске қосылды</b>\n"
